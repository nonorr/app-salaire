node{
      ...
      stage('Connexion ssh'){
        ...
        sh 'echo "192.168.158.107 app-salaire.rais-nordine.form" >> /etc/hosts'
        sh 'apk add sshpass'
        sh 'rm -fr /root/.ssh/*'
        sh 'ssh-keygen -q -t rsa -N \'\' -f "/root/.ssh/id_rsa"'
        sh 'sshpass -p \'123456\' ssh-copy-id  -o stricthostkeychecking=no "root@app-salaire.dimitri.form"'
